...
class ScannerPersistenceService:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_scan_results_created_at ON scan_results(created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_scan_results_scan_type ON scan_results(scan_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_code_issues_scan_id ON code_issues(scan_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_code_issues_severity ON code_issues(severity)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_code_issues_file_path ON code_issues(file_path)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_file_analysis_scan_id ON file_analysis(scan_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_file_analysis_language ON file_analysis(language)")
        ...
        try:
            ...
            if success:
                self.event_bus.publish(Event(
                    type=EventType.SCAN_COMPLETED if scan_result.status == ScanStatus.COMPLETED else EventType.SCAN_STARTED,
                    data={"scan_id": scan_result.scan_id, "status": scan_result.status.value}
                ))
            ...
            self.error_handler.handle_error(f"Scan result save failed: {e}", ErrorSeverity.ERROR)
            return False
    ...
    def get_code_issues(self, scan_id: str, severity: Optional[IssueSeverity] = None) -> List[CodeIssue]:

...
from src.backend.services.continuous_learning import get_continuous_learning_service, FeedbackType
...
def continuous_learning_backend(operation: str, **kwargs):
    ...
    if operation == "trigger_update":
        ...
        result = {"success": True, "deleted_count": 25}
    else:
...
class ContinuousLearningTab(QWidget):
    ...
    def create_model_management_tab(self) -> QWidget:
        ...
        self.update_history_table.horizontalHeader().setSectionResizeMode(
    ...
    def refresh_stats(self):
        ...
        try:
            ...
            for update in history:
                item_text = (
                    f"{update.timestamp.strftime('%Y-%m-%d %H:%M')} - {update.status}"
                )
            ...
            for i, update in enumerate(history):
                ...
                self.update_history_table.setItem(
                    i, 1, QTableWidgetItem(update.timestamp.strftime("%Y-%m-%d %H:%M"))
                )
                ...
                self.update_history_table.setItem(
                    ...
                    3,
                    QTableWidgetItem(
                        f"{update.samples_accepted}/{update.samples_processed}"
                    ),
                )
    ...
    def submit_feedback(self):
        ...
        try:
            ...
            if not original_input or not original_output:
                QMessageBox.warning(
                    self, "Warning", "Please provide both original input and output."
                )
                return
            ...
            QMessageBox.information(
                self, "Success", f"Feedback submitted successfully! ID: {feedback_id}"
            )
        ...
        try:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                self.start_continuous_learning(
                    "trigger_update", batch_size=batch_size, force_update=force_update
                )
    ...
    def cleanup_data(self):
        ...
        try:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                self.start_continuous_learning(
                    "cleanup_data", days_to_keep=days_to_keep
                )
    ...
    def start_continuous_learning(self, operation: str, **kwargs):
        ...
        self.worker = self.executor.submit(
            continuous_learning_backend, operation, **kwargs
        )
    ...
    def on_training_complete(self, result: Optional[Dict[str, Any]]):
        ...
        if result and result.get("success"):
            QMessageBox.information(
                self, "Training Complete", "Model training finished successfully."
            )
            ...
            QMessageBox.critical(
                self, "Training Failed", f"Model training failed: {error_msg}"
            )

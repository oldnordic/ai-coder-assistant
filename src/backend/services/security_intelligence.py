...
class SecurityIntelligenceService:
    ...
    def __init__(self):
        ...
        # Load configuration
        self.max_file_size_kb = self._config_manager.get(
            "security.max_file_size_kb", MAX_FILE_SIZE_KB
        )
        self.analysis_timeout = self._config_manager.get(
            "security.timeout_seconds", ANALYSIS_TIMEOUT_SECONDS
        )
    ...
    def load_config(self):
        ...
        try:
            config_path = self._config_manager.get_config_file_path(
    ...
    def save_config(self):
        ...
        try:
            config_data = {"feeds": [feed.__dict__ for feed in self.feeds.values()]}
    ...
    def save_data(self):
        ...
        try:
            ...
            with open("data/security_vulnerabilities.json", "w") as f:
                json.dump(
                    [vuln.__dict__ for vuln in self.vulnerabilities.values()],
                    f,
                    indent=2,
                    default=str,
                )
            ...
            with open("data/security_breaches.json", "w") as f:
                json.dump(
                    [breach.__dict__ for breach in self.breaches.values()],
                    f,
                    indent=2,
                    default=str,
                )
            ...
            with open("data/security_patches.json", "w") as f:
                json.dump(
                    [patch.__dict__ for patch in self.patches.values()],
                    f,
                    indent=2,
                    default=str,
                )
    ...
    def search_vulnerabilities(self, query: str) -> List[SecurityVulnerability]:
        ...
        for vuln in self.vulnerabilities.values():
            if (
                query_lower in vuln.title.lower()
                or query_lower in vuln.description.lower()
                or query_lower in vuln.id.lower()
            ):
                results.append(vuln)

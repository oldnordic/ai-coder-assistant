...
from PyQt6.QtWidgets import (
    QDialog, QVBoxLayout, QGroupBox, QFormLayout, QLineEdit, QPushButton, QLabel, QListWidget, QHBoxLayout, QMessageBox, QComboBox, QWidget, QCheckBox, QDialogButtonBox
)
from src.backend.services.pr_automation import PRTemplate, ServiceConfig
...
class ServiceConfigDialog(QDialog):
    ...
    def on_service_type_changed(self, service_type: str):
        ...
        if service_type == "JIRA":
            ...
            self.config_layout.labelForField(self.project_key_edit).setText(
                "Project Key (JIRA):"
            )
        else:
    ...
    def test_connection(self):
        ...
        QMessageBox.information(
            self, "Test Connection", "Connection test would be implemented here."
        )
    ...
    def get_config(self) -> ServiceConfig:
        ...
        return ServiceConfig(
            ...
            project_key=(
                self.project_key_edit.text() if service_type == "jira" else None
            ),
...
class PRTemplateDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.title_template_edit.setPlaceholderText(
            "Available variables: {title}, {jira_ticket}, {servicenow_ticket}"
        )
        ...
        self.body_template_edit.setPlaceholderText(
            "Available variables: {description}, {jira_ticket}, {servicenow_ticket}, {branch_name}, {base_branch}"
        )
    ...
    def get_template(self) -> PRTemplate:
        ...
        labels = [
            label.strip()
            for label in self.labels_edit.text().split(",")
            if label.strip()
        ]
        reviewers = [
            reviewer.strip()
            for reviewer in self.reviewers_edit.text().split(",")
            if reviewer.strip()
        ]
...
class PRCreationWidget(QWidget):
    ...
    def create_pr(self):
        ...
        QMessageBox.information(
            self, "Create PR", "PR creation would be implemented here."
        )
...
class PRManagementTab(QWidget):
    ...
    def refresh_services(self):
        ...
        self.add_service_row(
            "JIRA-Prod", "JIRA", "https://company.atlassian.net", "Connected"
        )
        self.add_service_row(
            "ServiceNow-Dev",
            "ServiceNow",
    ...
    def edit_service(self):
        ...
        if current_row >= 0:
            ...
            QMessageBox.information(
                self, "Edit Service", f"Edit service: {service_name}"
            )
    ...
    def test_service_by_name(self, service_name: str):
        ...
        QMessageBox.information(
            self, "Test Connection", f"Testing connection to {service_name}"
        )
    ...
    def edit_template(self):
        ...
        if current_item:
            ...
            QMessageBox.information(
                self, "Edit Template", f"Edit template: {template_name}"
            )
    ...
    def set_default_template(self):
        ...
        if current_item:
            ...
            QMessageBox.information(
                self, "Set Default", f"Set {template_name} as default template"
            )

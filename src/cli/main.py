...
def analyze_file(
    file_path: str, language: str, output_format: str = "text"
) -> Dict[str, Any]:
...
def scan_workspace(
    workspace_path: str, output_file: Optional[str] = None, output_format: str = "json"
) -> Dict[str, Any]:
...
def security_scan(
    ...
    try:
        ...
        for issue in results:
            if (
                "security" in issue.get("issue_type", "").lower()
                or "vulnerability" in issue.get("issue_type", "").lower()
            ):
        ...
        if fail_on_critical and security_results["critical_issues"]:
            print(
                f"Critical security issues found: {len(security_results['critical_issues'])}"
            )
            sys.exit(1)
...
def create_pr(
    ...
    repo_path: str,
    pr_type: str = "general",
    priority_strategy: str = "severity",
...
def main():
    ...
    security_parser = subparsers.add_parser(
        "security-scan", help="Perform security scan"
    )
    ...
    security_parser.add_argument(
        "--fail-on-critical",
        action="store_true",
        help="Exit with error on critical issues",
    )

    # Create PR command
    pr_parser = subparsers.add_parser(
        "create-pr", help="Create pull request from scan results"
    )
    ...
    pr_parser.add_argument(
        "--priority-strategy", default="severity", help="Priority strategy"
    )
    pr_parser.add_argument(
        "--auto-commit", action="store_true", help="Auto-commit changes"
    )
    pr_parser.add_argument("--create-pr", action="store_true", help="Create PR")

    # LLM Studio commands
    llm_studio_parser = subparsers.add_parser(
        "llm-studio", help="LLM Studio management"
    )
    llm_studio_subparsers = llm_studio_parser.add_subparsers(
        dest="llm_command", help="LLM Studio commands"
    )

    # LLM Studio status
    status_parser = llm_studio_subparsers.add_parser(
        "status", help="Check LLM Studio status"
    )

    # LLM Studio add provider
    add_provider_parser = llm_studio_subparsers.add_parser(
        "add-provider", help="Add a provider to LLM Studio"
    )
    add_provider_parser.add_argument(
        "--provider",
        required=True,
        help="Provider name (openai, anthropic, google_gemini, ollama)",
    )
    add_provider_parser.add_argument(
        "--api-key", required=True, help="API key or endpoint URL"
    )
    add_provider_parser.add_argument("--base-url", help="Base URL (optional)")

    # LLM Studio list providers
    list_providers_parser = llm_studio_subparsers.add_parser(
        "list-providers", help="List configured providers"
    )

    # LLM Studio test provider
    test_provider_parser = llm_studio_subparsers.add_parser(
        "test-provider", help="Test provider connection"
    )
    test_provider_parser.add_argument(
        "--provider", required=True, help="Provider name to test"
    )
    ...
    if args.command == "analyze":
        ...
        result = security_scan(
            args.workspace, args.output, args.format, args.fail_on_critical
        )
        if "error" in result:
            print(f"Error: {result['error']}", file=sys.stderr)
            sys.exit(1)

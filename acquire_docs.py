...
def crawl_and_extract_docs(source_config, save_base_dir, progress_callback=None, log_message_callback=None):
    ...
    if not os.path.exists(save_dir):
        ...
        if log_message_callback: log_message_callback(f"Created directory: {save_dir}/")

    visited_urls, urls_to_visit, page_count = set(), [start_url], 0
    if log_message_callback: log_message_callback(f"Starting crawl for {source_config['name']}...")

    with tqdm(total=max_pages, desc=f"Crawling {source_config['name']}", disable=(progress_callback is not None)) as pbar:
        while urls_to_visit and page_count < max_pages:
            current_url = urls_to_visit.pop(0)
            if current_url in visited_urls: continue
            visited_urls.add(current_url)
            
            if progress_callback: progress_callback(page_count, max_pages, f"Crawling Page {page_count}/{max_pages}")
            ...
            try:
                ...
                if log_message_callback: log_message_callback(f"Request error for {current_url}: {e}")

    if log_message_callback: log_message_callback(f"Crawl for {source_config['name']} finished.")
...
def acquire_all_documentation(progress_callback=None, log_message_callback=None):
    ...
    for source_config in DOC_SOURCES:
        crawl_and_extract_docs(source_config, os.path.join(os.getcwd(), config.BASE_DOCS_SAVE_DIR), progress_callback, _log)
        time.sleep(1)

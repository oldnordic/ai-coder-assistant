...
class ModelPersistenceService:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_configs_type ON model_configs(model_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_configs_name ON model_configs(name)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_states_model_id ON model_states(model_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_states_timestamp ON model_states(timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_states_state ON model_states(state)")
    ...
    def save_model_config(self, config: ModelConfig) -> bool:
        ...
        try:
            ...
            self.error_handler.handle_error(f"Model config save failed: {e}", ErrorSeverity.ERROR)
            return False
    ...
    def save_model_state(self, state_record: ModelStateRecord) -> bool:
        ...
        try:
            ...
            if success:
                self.event_bus.publish(Event(
                    type=EventType.MODEL_LOADED if state_record.state == ModelState.LOADED else EventType.MODEL_UNLOADED,
                    data={"model_id": state_record.model_id, "state": state_record.state.value}
                ))
                self.logger.info(f"Saved model state: {state_record.model_id} -> {state_record.state.value}")
            return success
        except Exception as e:
            self.logger.error(f"Failed to save model state {state_record.model_id}: {e}")
            return False
    ...
    def get_model_states(self, model_id: str, limit: int = 100) -> List[ModelStateRecord]:
        ...
        def get_states_operation(conn: sqlite3.Connection) -> List[ModelStateRecord]:
            ...
            states = []
            for row in cursor.fetchall():
                states.append(ModelStateRecord(
                    model_id=row[1],

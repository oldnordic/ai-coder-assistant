...
class OllamaClient:
    ...
    async def delete_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error deleting Ollama model {model_name}: {e.response.status_code} - {e.response.text}"
            )
            return False
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while deleting model {model_name}: {e}"
            )
            return False

    async def generate(
        self, model: str, prompt: str, stream: bool = False, temperature: float = 0.7
    ) -> str:
...
def _extract_json_from_response(response_text: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Could not fix JSON issues in response: {json_text[:200]}..."
            )
            return ""
...
async def analyze_code(
    client: OllamaClient, content: str, model_name: str
) -> List[Dict[str, Any]]:
...
"""
    ...
    try:
        ...
        logger.error(
            f"An unexpected error occurred during code analysis: {e}", exc_info=True
        )
        return []
...
async def get_suggestion_for_issue(
    client: OllamaClient, issue: Dict[str, Any], model_name: str
) -> str:
...
"""
    ...
    try:
        suggestion = await client.generate(
            model=model_name, prompt=prompt, temperature=0.2
        )
...
def get_suggestion_for_issue_sync(
    client: OllamaClient, issue: Dict[str, Any], model_name: str
) -> str:
...
async def get_suggestions_for_issues_batch(
    client: OllamaClient, issues: List[Dict[str, Any]], model_name: str
) -> List[Dict[str, Any]]:
...
"""
    ...
    try:
        ...
        logger.error(
            f"An unexpected error occurred during batch suggestion: {e}", exc_info=True
        )
        return []

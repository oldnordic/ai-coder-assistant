...
from .ollama_client import OllamaClient, get_suggestions_for_issues_batch
        ...
        try:
            ...
            if cache_file.exists():
                with open(cache_file, "r", encoding="utf-8") as f:
                    ...
                    if cached_data.get("timestamp", 0) > (
                        time.time() - CACHE_EXPIRY_SECONDS
                    ):
...
def generate_report_and_training_data(
    suggestion_list, model_mode, model_ref, tokenizer_ref, **kwargs
):
    ...
    if len(suggestion_list) > max_suggestions:
        log_message_callback(
            f"Limiting suggestions to {max_suggestions} to prevent timeout..."
        )
    ...
    try:
        ...
        log_message_callback(
            f"AI explanations completed: {len(explanations)} explanations generated"
        )
        ...
        explanations = [
            f"AI analysis for issue: {s.get('description', 'Unknown issue')}"
    ...
    log_message_callback(
        f"Summary stats: {len(issues_by_type)} types, {len(issues_by_severity)} severities"
    )
    ...
    for severity in severity_order:
        if severity in issues_by_severity:
            md_parts.append(
                f"- **{severity.title()}:** {issues_by_severity[severity]}\n"
            )
    ...
    for i, (suggestion, explanation) in enumerate(zip(suggestion_list, explanations)):
        log_message_callback(
            f"Processing issue {i+1}/{total_issues}: {suggestion.get('description', 'Unknown')[:50]}..."
        )
        progress_callback(
            i + 1, total_issues, f"Building report section {i+1}/{total_issues}..."
        )
        ...
        # Add issue section
        md_parts.append(
            f"### {i+1}. {issue_type.replace('_', ' ').title()} - {severity.title()} Severity\n\n"
        )
        ...
        if original_code:
            ...
            md_parts.append(
                f"```{suggestion.get('language', 'text')}\n{original_code}\n```\n\n"
            )
...
def get_ai_explanation(suggestion, model_mode, model_ref, tokenizer_ref, **kwargs):
    ...
    try:
        # Check cache first
        cache_key = (
            f"{suggestion['description']}_{suggestion.get('issue_type', 'unknown')}"
        )
...
def _generate_fallback_explanation(suggestion):
    ...
    fallback = fallback_explanations.get(
        issue_type,
        f"This suggested improvement addresses the identified issue: {description}",
    )
...
def browse_web_tool(url: str, **kwargs) -> str:
    ...
    try:
        _progress(
            PROGRESS_MIN, PROGRESS_MAX, f"Starting enhanced web scraping of {url}"
        )
        ...
        def extract_navigation_links(soup, current_url: str) -> list:
            ...
            try:
                ...
                log_message_callback(
                    f"Extracted {len(links)} links from {current_url}:"
                )
                for l in links:
        ...
        def should_follow_link(link_url: str, current_depth: int) -> bool:
            if current_depth >= max_depth:
                log_message_callback(
                    f"SKIP: {link_url} (max depth {max_depth} reached)"
                )
                return False
        ...
        def scrape_single_page(page_url: str, depth: int = 0) -> tuple:
            ...
            try:
                ...
                # Create a new session for each request to avoid connection pooling
                # issues
                with requests.Session() as session:
                    ...
                    links = (
                        extract_navigation_links(soup, page_url)
                        if depth < max_depth
                        else []
                    )

                    log_message_callback(
                        f"Successfully scraped {len(cleaned_text)} characters from {page_url}"
                    )
        ...
        if main_content:
            ...
            log_message_callback(
                f"Main page scraped successfully ({len(main_content)} characters)"
            )
        else:
        ...
        while (
            links_to_visit and pages_scraped < max_pages and current_depth <= max_depth
        ):
            # Check cancellation more frequently
            if cancellation_callback():
                log_message_callback("Web scraping cancelled by user")
            ...
            if time.time() - start_time > timeout:
                log_message_callback(
                    f"Timeout reached during link following after {timeout} seconds"
                )
                break
            ...
            log_message_callback(
                f"Processing {len(current_links)} links at depth {current_depth}"
            )
            ...
            for link in current_links:
                ...
                _progress(
                    pages_scraped,
                    max_pages,
                    f"Scraping page {pages_scraped + 1}/{max_pages}: {link}",
                )
        ...
        if len(final_content) > MAX_CONTENT_SIZE:  # 50KB limit
            final_content = (
                final_content[:MAX_CONTENT_SIZE]
        ...
        _progress(
            PROGRESS_COMPLETE,
            PROGRESS_MAX,
            f"Web scraping completed: {pages_scraped} pages",
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def yt_dlp_progress_hook(d):
                if d["status"] == "downloading":
                    ...
                    if total_bytes > 0:
                        percentage = (
                            d["downloaded_bytes"] / total_bytes
                        ) * PERCENTAGE_MULTIPLIER
                        _progress(
                            int(percentage * PROGRESS_WEIGHT_DOWNLOAD),
                            PROGRESS_MAX,
                            f"Downloading audio... {d['_percent_str']}",
                        )
...
def batch_process_suggestions(
    ...
    **kwargs,
    ...
    for batch_num, i in enumerate(range(0, total_suggestions, batch_size)):
        ...
        try:
            batch_explanations = _process_suggestion_batch(
                batch, model_mode, model_ref, tokenizer_ref, **kwargs
            )
            explanations.extend(batch_explanations)
            log_message_callback(
                f"Batch {batch_num} completed: {len(batch_explanations)} explanations generated"
            )
        except Exception as e:
            ...
            log_message_callback(
                f"Batch {batch_num} traceback: {traceback.format_exc()}"
            )
    ...
    **kwargs,
    ...
    for suggestion in batch:
        try:
            ...
            explanations.append(
                f"AI analysis for issue: {suggestion.get('description', 'Unknown issue')}"
            )
...
class AITools:
    ...
    def generate_with_own_model(
        self, model, tokenizer, prompt: str, max_new_tokens: int = 50
    ) -> str:
    ...
    def batch_process_suggestions(
        ...
        **kwargs,
        ...
        return batch_process_suggestions(
            suggestions, model_mode, model_ref, tokenizer_ref, **kwargs
        )

    def generate_report_and_training_data(
        self, suggestion_list, model_mode, model_ref, tokenizer_ref, **kwargs
    ):
        """Generate report and training data."""
        return generate_report_and_training_data(
            suggestion_list, model_mode, model_ref, tokenizer_ref, **kwargs
        )
...
def _generate_own_model_explanation(
    suggestion, model_ref, tokenizer_ref, log_message_callback
):
...
def run_build_and_test_in_docker(
    ...
    run_ok, run_out = run_docker_container(
        image=tag, run_args=run_args_list, command=test_cmd, detach=False
    )
    return {"success": run_ok, "stage": "test", "output": run_out}
...
async def enhance_issues_with_ai_async(
    ...
    if model_mode != "ollama":
        _log(
            f"AI enhancement is only supported for 'ollama' mode, but got '{model_mode}'. Skipping."
        )
        return issues
    ...
    try:
        ...
        for i in range(0, total_issues, BATCH_SIZE):
            ...
            batch_indices = {
                ...
                if i <= original_index < i + BATCH_SIZE
            }

            _log(
                f"Processing batch {i//BATCH_SIZE + 1}/{-(-total_issues//BATCH_SIZE)}..."
            )

            try:
                suggestions = await get_suggestions_for_issues_batch(
                    client, batch, model_ref
                )
                ...
                for suggestion in suggestions:
                    ...
                    if (
                        ...
                        and i <= original_index < i + BATCH_SIZE
                    ):
                ...
                if progress_callback:
                    progress_callback(
                        min(i + BATCH_SIZE, total_issues),
                        total_issues,
                        f"Enhanced {min(i + BATCH_SIZE, total_issues)}/{total_issues} issues...",
                    )

...
class AnalysisWidget(QWidget):
    ...
    def browse_file(self) -> None:
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select File to Analyze", "", "Python Files (*.py);;All Files (*)"
        )
        if file_path:
    ...
    def analyze_target(self) -> None:
        ...
        if not file_path and not dir_path:
            QMessageBox.warning(
                self, "Warning", "Please select a file or directory to analyze."
            )
            return

        if file_path and dir_path:
            QMessageBox.warning(
                self, "Warning", "Please select either a file or directory, not both."
            )
            return
        ...
        backend_func = (
            ...
            if file_path
            else analyze_directory_performance_backend
        )
        ...
        try:
            ...
            worker = self.thread_manager.start_worker(
                self.worker_id, backend_func, target_path
            )
            ...
            if "directory_path" in result:
                ...
                for issue in result["issues"]:
                    ...
                    self.issues_table.setItem(
                        row, 0, QTableWidgetItem(str(issue["line"]))
                    )
                    self.issues_table.setItem(row, 1, QTableWidgetItem(issue["type"]))
                    self.issues_table.setItem(
                        row, 2, QTableWidgetItem(issue["severity"])
                    )
                    self.issues_table.setItem(
                        row, 3, QTableWidgetItem(issue["description"])
                    )

...
def test_on_task_failed(task_management_component):
    errors = []
    task_management_component.error_occurred.connect(lambda msg, sev: errors.append((msg, sev)))
    event = Event(type=EventType.TASK_FAILED, data={"task_id": TEST_TASK_ID_456, "error": "Test error"})

...
def test_on_model_error(model_management_component):
    errors = []
    model_management_component.error_occurred.connect(lambda msg, sev: errors.append((msg, sev)))
    event = Event(type=EventType.MODEL_ERROR, data={"model_id": "cloud", "error": "Test error"})

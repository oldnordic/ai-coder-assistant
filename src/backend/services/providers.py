        ...
        try:
            ...
            for model in response.data:
                model_config = ModelConfig(
                    name=model.id,
                    provider=ProviderType.OPENAI,
                    model_type=ModelType.CHAT if "gpt" in model.id else ModelType.COMPLETION,
    ...
    async def chat_completion(self, request: ChatCompletionRequest) -> ChatCompletionResponse:
        ...
        try:
            ...
            return ChatCompletionResponse(
                ...
                usage={"prompt_tokens": len(str(messages)), "completion_tokens": len(response.text)},
                finish_reason="stop",
                provider=ProviderType.GOOGLE_GEMINI,
                response_time=response_time,
                cost=self.calculate_cost({"prompt_tokens": len(str(messages)), "completion_tokens": len(response.text)}, request.model)
            )
        ...
        try:
            ...
            for model_name, model_type, context_length, cost in gemini_models:
                model_config = ModelConfig(
                    ...
                    capabilities=["chat", "vision"] if "vision" in model_name else ["chat"]
                )
                models.append(model_config)

...
def ollama_backend(operation: str, **kwargs) -> Dict[str, Any]:
    ...
    if operation == "list_models":
        ...
        return {"success": True, "model_name": model_name, "status": "downloaded"}
    elif operation == "delete_model":
        model_name = kwargs.get("operation_args", {}).get("model_name", "unknown")
        return {"success": True, "model_name": model_name, "status": "deleted"}
    ...
    def populate_models_table(self, models: List[Any]):
        ...
        for i, model_data in enumerate(models):
            ...
            if hasattr(model_data, "name"):
                ...
                size_gb = (
                    f"{size_bytes / (1024**3):.2f} GB" if size_bytes > 0 else "N/A"
                )
            else:
                # Dictionary fallback
            ...
            delete_btn.clicked.connect(
                lambda checked, m=model_name: self.delete_model(m)
            )
            ...
            export_btn.clicked.connect(
                lambda checked, m=model_name: self.export_model(m)
            )
    ...
    def delete_model(self, model_name: str):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.executor.submit(
                self.llm_manager.delete_model, model_name
            ).add_done_callback(self._on_delete_complete)

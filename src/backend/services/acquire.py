...
def process_url_parallel(
    ...
    try:
        ...
        if content and not content.startswith("Error"):
            ...
            pages_estimate = content.count("=== MAIN PAGE ===") + content.count(
                "=== RELATED PAGE ==="
            )
            _log(
                f"Content stats: {content_length} characters, approximately {pages_estimate} pages"
            )
...
def crawl_docs(urls: List[str], output_dir: str, **kwargs: Any) -> Dict[str, Any]:
    ...
    progress_callback: ProgressCallback = kwargs.get(
        "progress_callback", lambda c, t, m: None
    )
    cancellation_callback: CancellationCallback = kwargs.get(
    ...
    log_message_callback(
        f"Starting enhanced crawl of {total_urls} URLs with multi-threading..."
    )
    log_message_callback(
        f"Configuration: max_pages={max_pages}, max_depth={max_depth}, same_domain_only={same_domain_only}, timeout={timeout}"
    )
    ...
    max_workers = max(
        1, min(multiprocessing.cpu_count(), total_urls, 4)
    )  # Cap at 4 threads for web scraping, minimum 1
    log_message_callback(
        f"Using {max_workers} worker threads for parallel web scraping"
    )
    ...
    with ThreadPoolExecutor(
        max_workers=max_workers, thread_name_prefix="web_scraper"
    ) as executor:
        ...
        for url in urls:
            if cancellation_callback():
                log_message_callback(
                    "Web scraping cancelled by user before processing all URLs"
                )
                break
    ...
    if successful_scrapes == 0:
        ...
        return summary
    log_message_callback(
        f"Enhanced web scraping process complete. Successfully scraped {successful_scrapes}/{total_urls} URLs."
    )
    return summary


def process_url_simple_parallel(
    url: str,
    output_dir: str,
...
def crawl_docs_simple(urls: list, output_dir: str, **kwargs):
    ...
    total_urls = len(urls)
    log_message_callback(
        f"Starting simple crawl of {total_urls} URLs with multi-threading..."
    )
    ...
    # Determine optimal number of workers
    max_workers = max(
        1, min(multiprocessing.cpu_count(), total_urls, 6)
    )  # Cap at 6 threads for web scraping, minimum 1
    log_message_callback(
        f"Using {max_workers} worker threads for parallel web scraping"
    )
    ...
    log_message_callback(
        f"Simple web scraping process complete. Successfully scraped {successful_scrapes}/{total_urls} URLs."
    )
    return summary

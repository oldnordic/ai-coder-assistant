...
def save_learning_feedback(suggestion_data, user_provided_code=None, **kwargs):
    ...
    try:
        ...
        good_code = (
            ...
            else suggestion_data.get("suggested_improvement", "").strip()
        )
        ...
        _log(
            f"Saved learning feedback: {suggestion_data.get('description', 'Unknown suggestion')}"
        )
    ...
    try:
        ...
        def update_progress(filename: str):
            ...
            with progress_lock:
                processed_count += 1
                progress_callback(
                    processed_count, len(files_to_process), f"Processing: {filename}"
                )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for filename in files_to_process:
                ...
                future = executor.submit(
                    process_file_parallel, filepath, filename, log_message_callback
                )
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    update_progress(filename)
        with open(
            settings.CONCAT_FILE_PATH, "a", encoding="utf-8", errors="ignore"
        ) as concat_file:
        ...
        if os.path.exists(settings.LEARNING_DATA_FILE):
            with open(
                settings.LEARNING_DATA_FILE, "r", encoding="utf-8", errors="ignore"
            ) as f:
                for line in f:
                    try:
                        finetuning_examples.append(json.loads(line))
        ...
        if valid_finetuning_strings:
            with open(
                settings.FINETUNING_FILE_PATH, "a", encoding="utf-8", errors="ignore"
            ) as f:

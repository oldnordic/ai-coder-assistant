...
class PythonPerformanceAnalyzer:
    ...
    def _calculate_impact_score(self, severity: str) -> float:
        ...
        severity_scores = {"critical": 1.0, "high": 0.8, "medium": 0.6, "low": 0.3}
        ...
        for category, category_issues in categories.items():
            if category == "algorithm":
                recommendations.append(
                    f"Algorithm optimization: {len(category_issues)} issues found. Consider reviewing loop structures and data structures."
                )
            elif category == "memory":
                recommendations.append(
                    f"Memory optimization: {len(category_issues)} issues found. Consider reducing memory allocations and improving garbage collection."
                )
            elif category == "io":
                recommendations.append(
                    f"I/O optimization: {len(category_issues)} issues found. Consider batching operations and using async I/O where appropriate."
                )

        # Add general recommendations
        if len(issues) > 10:
            recommendations.append(
                "High number of performance issues detected. Consider a comprehensive code review."
            )
...
class PerformanceOptimizationService:
    ...
    def __init__(self):
        ...
        self.max_file_size_kb = self.config.get(
            "performance.max_file_size_kb", MAX_FILE_SIZE_KB
        )
        self.analysis_timeout = self.config.get(
            "performance.timeout_seconds", ANALYSIS_TIMEOUT_SECONDS
        )

        self.logger.info("Performance optimization service initialized")

        self.analyzers = {"python": PythonPerformanceAnalyzer()}
        self.analysis_cache = {}
    ...
    def analyze_file(self, file_path: str) -> PerformanceAnalysisResult:
        ...
        if language not in self.analyzers:
            return PerformanceAnalysisResult(
                ...
                issues=[],
                recommendations=[
                    f"Language {language} not supported for performance analysis"
                ],
            )
    ...
    def get_analysis_summary(
        self, results: List[PerformanceAnalysisResult]
    ) -> Dict[str, Any]:
...
class PersistencePerformanceMonitor:
    ...
    def get_summary(self):
        with self.lock:
            ...
            avg_time = (
                (sum(m["duration"] for m in self.metrics) / total) if total else 0.0
            )
            return {
    ...
    try:
        ...
        if log_message_callback:
            log_message_callback(
                f"Directory analysis complete. Analyzed {len(results)} files."
            )

...
class PythonASTVisitor(NodeVisitor):
    ...
    def _check_naming_convention(
        self, node: Union[ast.FunctionDef, ast.ClassDef, ast.Name], node_type: str
    ) -> None:
        ...
        for rule in self.rules:
            if (
                ...
                and rule.language == Language.PYTHON
                and rule.enabled
            ):
            ...
            if (
                ...
                if isinstance(node.value, (int, float)):
                    ...
                    if re.match(rule.pattern, value):
                        self._add_violation(
                            rule, node.lineno, node.col_offset, f"Magic number: {value}"
                        )
...
class CodeStandardsService:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                for standard_data in data.get("standards", []):
                    ...
                    if "created_date" in standard_data and isinstance(
                        standard_data["created_date"], str
                    ):
                        try:
                            standard_data["created_date"] = datetime.fromisoformat(
                                standard_data["created_date"]
                            )
                        except ValueError:
                            standard_data["created_date"] = datetime.now()

                    if "last_updated" in standard_data and isinstance(
                        standard_data["last_updated"], str
                    ):
                        try:
                            standard_data["last_updated"] = datetime.fromisoformat(
                                standard_data["last_updated"]
                            )
                        except ValueError:
        ...
        if standard_name in self.standards:
            ...
            if self._current_standard_name == standard_name:
                self._current_standard_name = (
                    list(self.standards.keys())[0] if self.standards else None
                )
            self.save_config()
    ...
    def analyze_file(self, file_path: str) -> "CodeAnalysisResult":
        ...
        try:
            ...
            result = CodeAnalysisResult(
                file_path=str(path), language=language, violations=violations
            )
            ...
            result.info_count = len(
                [v for v in violations if v.severity == Severity.INFO]
            )
    ...
    def _analyze_python_file(
        self, content: str, rules: List["CodeRule"], file_path: str
    ) -> List["CodeViolation"]:
